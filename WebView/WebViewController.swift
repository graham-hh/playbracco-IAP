...
// MARK: - Programmatic Pro Shop Screen
final class BraccoProShopViewController: UIViewController {
...
var persistentScanningMode: Bool {
    get { UserDefaults.standard.object(forKey: "persistentScanningMode") as? Bool ?? false }
    set { UserDefaults.standard.set(newValue, forKey: "persistentScanningMode") }
}
...
class WebViewController: UIViewController, OSSubscriptionObserver, GADBannerViewDelegate, SKStoreProductViewControllerDelegate,SKProductsRequestDelegate, SKPaymentTransactionObserver, FBInterstitialAdDelegate, GADFullScreenContentDelegate, WKUIDelegate
{
    ...
    // --- Shop bottom sheet overlay (blocks touches below) ---
    private var shopDimmingView: UIView?
    private var shopSheetView: UIView?
    private var shopSheetBottomConstraint: NSLayoutConstraint?
    ...
    {
        super.viewDidLoad()
        // Restore Rookie/Pro mode from UserDefaults before setting up the webview
        let lastMode = UserDefaults.standard.object(forKey: "lastMode") as? Bool
        if let lastMode = lastMode {
            UserManager.shared.isRookieMode = lastMode
            print("🎮 Restored last known mode from UserDefaults: \(lastMode ? "Rookie" : "Pro")")
        }
        ...
        (function() {
          ...
          function onTargetPage() {
            try {
              ...
              if (h !== ALLOWED_HOST) return false;
              for (var i = 0; i < PATH_MATCHES.length; i++) {
                if (p.indexOf(PATH_MATCHES[i]) !== -1) return true;
              }
            } catch (e) {}
            return false;
          }
          ...
          function hideButtonsByText(texts) {
            ...
            nodes.forEach(function(n) {
              ...
              texts.forEach(function(txt) {
                if (t === (txt || "").toLowerCase()) hideNode(n);
              });
            });
          }
          ...
          function run() {
            ...
            // TODO: Replace with your actual selectors and labels
            hideBySelectors([
              '#theButtonId',
              'button[data-test="join-now"]',
              '.close-account-btn',
              '.MuiBox-root.css-1qr23p0'
            ]);
            hideButtonsByText(['Join Now', 'Close Account']);
          }

          // Initial pass
          run();

          // Re-apply for SPA/modal changes
          if (window.__wvgHideObserver) { try { window.__wvgHideObserver.disconnect(); } catch(e) {} }
        ...
        let playSlipJS = """
        (function() {
          // Try a few ways to open the Play Slip (sidebar)
          function openSlip() {
            try {
              // 1) If the app exposes a global method
              if (window.openPlaySlip) { window.openPlaySlip(); return true; }

              // 2) Click a known button (add your real selectors here as you discover them)
              var btn = document.querySelector('[data-testid="playslip-open"], [data-test="playslip-open"], button#playslip, button[aria-label="Play Slip"]');
              if (btn) { btn.click(); return true; }

              // 3) Click by visible text
              var candidates = Array.from(document.querySelectorAll('button, a, [role="button"]'));
              var target = candidates.find(function(n){
                var t = (n.innerText||n.textContent||"").trim().toLowerCase();
                return t === "play slip" || t === "playslip" || t.indexOf("slip") !== -1;
              });
          ...
          // Try to pin the mini/collapsed slip above the native tab bar
          function pinMiniSlip() {
            try {
              // Adjust these selectors to your DOM
              var mini = document.querySelector('.playslip-mini, .sidebar-mini, .betslip-floating, [data-testid="mini-slip"]');
              if (!mini) return false;

              // Ask native for the tab bar height (we pass it from Swift); fallback to 56
              var height = window.__wvg_tabbar_h || 56;

              mini.style.position = 'fixed';
              mini.style.left = '0';
              mini.style.right = '0';
              mini.style.bottom = (height + 6) + 'px';
          ...
          // Expose helpers
          window.wvgOpenPlaySlip = function() { return openSlip(); };
          window.wvgPinMiniSlip  = function(h) { window.__wvg_tabbar_h = h; return pinMiniSlip(); };

          // Re-apply on SPA/nav/modal mutations
          if (window.__wvgSlipObs) { try { window.__wvgSlipObs.disconnect(); } catch(e){} }
          window.__wvgSlipObs = new MutationObserver(function() {
            pinMiniSlip();
          });
        ...
        config.userContentController.addUserScript(playSlipScript)
        let accountHideScript = WKUserScript(source: accountHideJS,
                                             injectionTime: .atDocumentEnd,
                                             forMainFrameOnly: true)
    ...
    // MARK: - WKScriptMessageHandler: BalanceUpdateProxy (Rookie/Pro mode detection)
    // This should be found in the message handler for "balances" or similar. Add the UserDefaults write.
    // (If this is handled in a delegate extension, the following code should be placed there.)
    // For illustration, here's the code to update/persist mode:
    //
    // UserManager.shared.isRookieMode = <detected_mode>
    // UserDefaults.standard.set(UserManager.shared.isRookieMode, forKey: "lastMode")

    // MARK: - WKNavigationDelegate
    // In webView(_:didFinish:), inject JS to post the current appState.selectedBalance to the bridge (to update mode)
    // Example:
    // let js = """
    //   if (window.appState && window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.balances) {
    //     window.webkit.messageHandlers.balances.postMessage({
    ...
    // MARK: - Shop Bottom Sheet (modal overlay that blocks interaction below)
    private func setupShopSheetIfNeeded(height: CGFloat = 340, dismissOnBackgroundTap: Bool = true) {
        guard shopDimmingView == nil && shopSheetView == nil else { return }

        // 1) Dimming overlay that intercepts touches
        let dim = UIView(frame: .zero)
        dim.translatesAutoresizingMaskIntoConstraints = false
        dim.backgroundColor = UIColor.black.withAlphaComponent(0.45)
        dim.alpha = 0.0
        dim.isUserInteractionEnabled = true           // blocks touches to underlying views
        dim.accessibilityViewIsModal = true           // better VoiceOver focus handling
        view.addSubview(dim)
        NSLayoutConstraint.activate([
        ...
        if dismissOnBackgroundTap {
            let tap = UITapGestureRecognizer(target: self, action: #selector(dismissShopBottomSheet))
            dim.addGestureRecognizer(tap)
        }
        self.shopDimmingView = dim
        ...
        let bottom = sheet.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: height)
        self.shopSheetBottomConstraint = bottom
        NSLayoutConstraint.activate([
            sheet.leadingAnchor.constraint(equalTo: view.leadingAnchor),
            sheet.trailingAnchor.constraint(equalTo: view.trailingAnchor),
            sheet.heightAnchor.constraint(equalToConstant: height),
            bottom
        ])
        self.shopSheetView = sheet
    }

    /// Presents the modal shop sheet and blocks interaction with the app beneath.
    /// If you already have a content view, pass it here; otherwise, add subviews to `shopSheetView` after presenting.
    @objc func presentShopBottomSheet(with contentView: UIView? = nil, height: CGFloat = 340) {
        setupShopSheetIfNeeded(height: height, dismissOnBackgroundTap: true)
        guard let dim = shopDimmingView, let sheetBottom = shopSheetBottomConstraint else { return }

        // Attach caller-provided content (optional)
        if let content = contentView, let sheet = shopSheetView {
            if content.superview !== sheet {
                ...
                NSLayoutConstraint.activate([
                    ...
                    content.topAnchor.constraint(equalTo: sheet.topAnchor),
                    content.bottomAnchor.constraint(equalTo: sheet.bottomAnchor)
                ])
            }
        }
        ...
        if let sheet = shopSheetView { view.bringSubviewToFront(sheet) }
    ...
    /// Dismisses the modal shop sheet and re-enables interaction below.
    @objc func dismissShopBottomSheet() {
        guard let dim = shopDimmingView, let sheetBottom = shopSheetBottomConstraint, let sheet = shopSheetView else { return }
        let off = sheet.bounds.height
        sheetBottom.constant = off
    ...
    func openPlaySlip() {
        webView?.evaluateJavaScript("window.wvgOpenPlaySlip && window.wvgOpenPlaySlip()") { result, error in
            if let error = error {
                print("PlaySlip open error:", error.localizedDescription)
            } else {
                print("PlaySlip open called:", result as Any)
            }
        }
    }

    // Call this after each navigation finish (and whenever layout changes) to pin mini slip above tab bar
    func pinMiniSlipAboveTabBar() {
        // If you know your native tab bar height, pass it along; 56 is your current default
        let barHeight: Int = 56
        let js = "window.wvgPinMiniSlip && window.wvgPinMiniSlip(\(barHeight))"
        webView?.evaluateJavaScript(js, completionHandler: nil)
    }
...
extension WebViewController: WKNavigationDelegate, UIDocumentInteractionControllerDelegate, EKEventEditViewDelegate
{
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!)
    {
        ...
        wvg_handleDidFinish(webView)   // ← add this line
...
/* --- CSS-based hider installed at document start (works before React renders) --- */
extension WebViewController {
    fileprivate func installCoinsHiderUserScript(on webView: WKWebView) {
        let css = """
        button[data-testid="account-dropdown-button"] { display: none !important; visibility: hidden !important; }
        """
        let js = """
        (function(){
          var style = document.createElement('style');
          style.setAttribute('data-native-style', 'hide-coins');
          style.innerHTML = `${CSS}`;
          (document.head || document.documentElement).appendChild(style);
        })();
        """.replacingOccurrences(of: "${CSS}", with: css.replacingOccurrences(of: "\n", with: " "))
        let script = WKUserScript(source: js, injectionTime: .atDocumentStart, forMainFrameOnly: false)
        webView.configuration.userContentController.addUserScript(script)
    }
    
    /// Injects a meta viewport (with viewport-fit=cover) and CSS that adds top padding equal
    /// to the device safe-area inset so the web app's top UI never sits under the status bar / notch.
    /// Runs at document start and applies to all pages (including modals opened later).
    fileprivate func installStatusBarSafeAreaStyle(on webView: WKWebView) {
        let css = """
        :root, body {
            /* Support both legacy and modern safe-area variables */
            padding-top: constant(safe-area-inset-top);
            padding-top: env(safe-area-inset-top);
            /* Add a tiny extra breathing room so text/icons don't kiss the notch */
            padding-top: calc(env(safe-area-inset-top, 0px) + 8px);
            box-sizing: border-box;
        }
        /* Prevent double scrollbars when we add padding */
        html, body { margin: 0; }
        """
        let js = """
        (function(){
          try {
            // Ensure viewport-fit=cover for proper env(safe-area-inset-*) values
            var hasMeta = false;
            var metas = document.getElementsByTagName('meta');
            for (var i=0;i<metas.length;i++){
              if (metas[i].name === 'viewport'){
                hasMeta = true;
                // If existing, append viewport-fit=cover if not present
                if (!/viewport-fit\\s*=\\s*cover/.test(metas[i].content || '')) {
                  metas[i].content = (metas[i].content ? metas[i].content + ', ' : '') + 'viewport-fit=cover';
                }
              }
            }
            if (!hasMeta) {
            ...
            // Inject CSS once
            if (!document.querySelector('style[data-native-style="safe-area-top-padding"]')) {
              var style = document.createElement('style');
              style.setAttribute('data-native-style', 'safe-area-top-padding');
              style.innerHTML = `${CSS}`;
              (document.head || document.documentElement).appendChild(style);
            }
          } catch(e) {}
...
// MARK: - Native-hides for specific web buttons (Purchase Bracco Coins)
extension WebViewController {
    /// Injects JS that hides the “Purchase Bracco Coins” button by a stable `data-testid`
    /// and by visible text. Works for elements added later (modals) via MutationObserver.
    fileprivate func injectHideCoinsButtonJS(into webView: WKWebView) {
        let js = """
        (function() {
          function hideTargets(root) {
            try {
              ...
              // 1) Prefer stable data-testid
              var btns = Array.from(scope.querySelectorAll('[data-testid="account-dropdown-button"]'));

              // 2) Fallback by visible text
              var textMatch = Array.from(scope.querySelectorAll('a,button,[role="button"]')).filter(function(el) {
                var t = (el.textContent || "").trim().toLowerCase();
                return t.includes("purchase bracco coins");
              });

              btns.concat(textMatch).forEach(function(el){
                if (!el.hasAttribute("data-hidden-by-native")) {
                  el.style.setProperty("display", "none", "important");
                  el.setAttribute("data-hidden-by-native", "true");
                }
              });
            } catch(e) {}
          }
          ...
          // Observe for future DOM changes (modals, dynamic content)
          try {
...
extension WebViewController
{
    func webView(_ webView: WKWebView, createWebViewWith configuration: WKWebViewConfiguration, for navigationAction: WKNavigationAction, windowFeatures: WKWindowFeatures) -> WKWebView? {
        ...
        if (google_login_helper_enabled || facebook_login_helper_enabled) {
            ...
            if let popup = loginPopupWindow {
                installCoinsHiderUserScript(on: popup)
                installStatusBarSafeAreaStyle(on: popup)
            }
...
extension WebViewController {
    func presentProShop() {
        let url = URL(string: "https://playbracco.com/bracco-coins")!
        let proVC = BraccoProShopViewController(externalURL: url)
